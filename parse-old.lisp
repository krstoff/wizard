...
(defstruct dot-call left name args)
(defstruct call name args)
(defstruct index-op left index)
(defstruct member-op left property)
(defstruct list-literal items)
(defstruct property-list-literal props)
(defstruct block statements)
(defstruct if-expr condition block else-block)
(defstruct while-expr condition block)
(defstruct loop-expr expr)
(defstruct cond-expr left clauses)
...
(defun parse-call-args ()
  (expect-token 'LEFT-PAREN (advance-token))
  (prog1
   (iterate
     (with rest-args = nil)
...
(defun parse-list-items (&aux first-keyword)
  (expect-token 'LEFT-BRACKET (advance-token))
  ...
  (cond
    ...
    ((binary-op-p token)
     ...
     (let ((name (make-sym :name token :path nil))
           (args `(:args (,left ,(expression (lbp token)))))))
     (make-call :name name :args args))
